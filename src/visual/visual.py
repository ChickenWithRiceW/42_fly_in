...
def position_calc(config: Config) -> Pos_values:
    ...
    min_val.x = min(zone.pos.x for zone in config.zones.values())
    max_val.x = max(zone.pos.x for zone in config.zones.values())

    min_val.y = min(zone.pos.y for zone in config.zones.values())
    max_val.y = max(zone.pos.y for zone in config.zones.values())
...
def map_prepping(config: Config, pos_values: Pos_values, action_log: list[list[tuple[str, str]]]):
    ...
    for drone in config.drones:
        drone.active = True
        drone.position = config.start_hub
        drone.next_step = config.start_hub
        config.start_hub.drones[drone.id] = drone

    bla_list = {}
    for drone in config.drones:
        bla_list.update({drone.id: config.start_hub})
...
def visual_worker(config: Config, action_logs: list[list[tuple[str, str]]]):
    ...
    print(len(config.start_hub.drones))
    while True:
